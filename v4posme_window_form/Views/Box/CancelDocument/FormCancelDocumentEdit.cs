...
public partial class FormCancelDocumentEdit : FormTypeHeadEdit, IFormTypeEdit
{
    #region Campos
    ...
    public TbComponent? ObjComponentCancelInvoice { get; set; }
    ...
    public FormCancelDocumentEdit()
    {
        InitializeComponent();
    }

    public FormCancelDocumentEdit(TypeOpenForm typeOpen, int companyId, int transactionMasterId, int transactionId)
    {
        InitializeComponent();
    ...
    private void FormCancelDocumentEdit_Load(object sender, EventArgs e)
    {
    ...
    public void ComandDelete()
    {
        ...
        var validateWorkflowStage = objInterfazCoreWebWorkflow.ValidateWorkflowStage("tb_transaction_master_cancel_invoice", "statusID", objTm.StatusId.Value, commandEliminable, user.CompanyID, user.BranchID, role.RoleID);
    ...
    public void ComandPrinter()
    {
        ...
        var objStage = objInterfazCoreWebWorkflow.GetWorkflowStage("tb_transaction_master_cancel_invoice", "statusID", objTM.StatusId.Value, user.CompanyID, user.BranchID, role.RoleID);
    ...
    public void LoadEdit()
    {
        ...
        if (appNeedAuthentication == "true")
        {
            ...
            var resultPermission = objInterfazCoreWebPermission.UrlPermissionCmd("app_box_canceldocument", "edit", urlSuffix!, role, user, VariablesGlobales.Instance.ListMenuTop, VariablesGlobales.Instance.ListMenuLeft, VariablesGlobales.Instance.ListMenuBodyReport, VariablesGlobales.Instance.ListMenuBodyTop, VariablesGlobales.Instance.ListMenuHiddenPopup);
            if (resultPermission == permissionNone)
            {
                throw new Exception(notAllEdit);
            }
        ...
        ObjComponentCancelInvoice = objInterfazCoreWebTools.GetComponentIdByComponentName("tb_transaction_master_cancel_invoice");
        if (ObjComponentCancelInvoice is null)
        {
        ...
        UrlPrinterDocument = objInterfazCoreWebParameter.GetParameterValue("BOX_CANCELDOCUMENT_URL_PRINTER", user.CompanyID);
        ...
        ObjListWorkflowStage = objInterfazCoreWebWorkflow.GetWorkflowStageByStageInit("tb_transaction_master_cancel_invoice", "statusID", ObjTransactionMaster.StatusId.Value, user.CompanyID, user.BranchID, role.RoleID);
    ...
    public void LoadNew()
    {
        ...
        if (appNeedAuthentication == "true")
        {
            ...
            var resultPermission = objInterfazCoreWebPermission.UrlPermissionCmd("app_box_canceldocument", "add", urlSuffix!, role, user, VariablesGlobales.Instance.ListMenuTop, VariablesGlobales.Instance.ListMenuLeft, VariablesGlobales.Instance.ListMenuBodyReport, VariablesGlobales.Instance.ListMenuBodyTop, VariablesGlobales.Instance.ListMenuHiddenPopup);
            if (resultPermission == permissionNone)
            {
        ...
        var transactionID = objInterfazCoreWebTransaction.GetTransactionId(user.CompanyID, "tb_transaction_master_cancel_invoice", 0);
        var objCurrency = objInterfazCoreWebCurrency.GetCurrencyDefault(user.CompanyID);
        var targetCurrency = objInterfazCoreWebCurrency.GetCurrencyExternal(user.CompanyID);

        ExchangeRate = objInterfazCoreWebCurrency.GetRatio(user.CompanyID, DateTime.Now.Date, decimal.One, targetCurrency.CurrencyID, objCurrency.CurrencyID);
        ObjCaudal = transactionCausalModel.GetCausalByBranch(user.CompanyID, transactionID.Value, user.BranchID);
        ObjListWorkflowStage = objInterfazCoreWebWorkflow.GetWorkflowInitStage("tb_transaction_master_cancel_invoice", "statusID", user.CompanyID, user.BranchID, role.RoleID);
    }
    ...
    public void SaveInsert()
    {
        ...
        objInterfazCoreWebPermission.GetValueLicense(user.CompanyID, "app_box_canceldocument/index");
        ...
        TransactionId = objInterfazCoreWebTransaction.GetTransactionId(user.CompanyID, "tb_transaction_master_cancel_invoice", 0) ?? 0;
        ...
        var objTM = new TbTransactionMaster
        {
            ...
            TransactionNumber = objInterfazCoreWebCounter.GoNextNumber(user.CompanyID, user.BranchID, "tb_transaction_master_cancel_invoice", 0),
            ...
            ComponentID = ObjComponentCancelInvoice.ComponentID,
            Note = txtNote.Text,
        ...
        var path = $"{pathFileOfApp}/company_{user.CompanyID}/component_{ObjComponentCancelInvoice.ComponentID}/component_item_{TransactionMasterId}";
    ...
    public void SaveUpdate()
    {
        ...
        if (appNeedAuthentication == "true")
        {
            ...
            resultPermission = objInterfazCoreWebPermission.UrlPermissionCmd("app_box_canceldocument", "edit", urlSuffix!, role, user, VariablesGlobales.Instance.ListMenuTop, VariablesGlobales.Instance.ListMenuLeft, VariablesGlobales.Instance.ListMenuBodyReport, VariablesGlobales.Instance.ListMenuBodyTop, VariablesGlobales.Instance.ListMenuHiddenPopup);
            if (resultPermission == permissionNone)
            {
                throw new Exception(notAllEdit);
            }
        ...
        //Obtener el Componente de Transacciones Facturacion
        var objComponentCancelInvoice = objInterfazCoreWebTools.GetComponentIdByComponentName("tb_transaction_master_cancel_invoice");
        if (objComponentCancelInvoice is null)
        {
            throw new Exception("EL COMPONENTE 'tb_transaction_master_cancel_invoice' NO EXISTE...");
        }
        ...
        var validateWorkflow = objInterfazCoreWebWorkflow.ValidateWorkflowStage("tb_transaction_master_cancel_invoice", "statusID", objTm.StatusId!.Value, commandEditableTotal, user.CompanyID, user.BranchID, role.RoleID);
        ...
        validateWorkflow = objInterfazCoreWebWorkflow.ValidateWorkflowStage("tb_transaction_master_cancel_invoice", "statusID", objTm.StatusId!.Value, commandEditable, user.CompanyID, user.BranchID, role.RoleID);
        if (validateWorkflow.HasValue && validateWorkflow.Value)
        {
        ...
        validateWorkflow = objInterfazCoreWebWorkflow.ValidateWorkflowStage("tb_transaction_master_cancel_invoice", "statusID", objTMNew.StatusID.Value, commandAplicable, user.CompanyID, user.BranchID, role.RoleID);
        if (validateWorkflow.HasValue && validateWorkflow.Value && objTm.StatusId.Value != objTMNew.StatusID)
        {
            ...
            if (objListTmd.Count > 0)
            {
                ...
                {
                    //Obtener Documento antes de aplicar la cuota
                    var objCustomerCreditDocumentInicial = customerCreditDocumentModel.GetRowByPk(objTmd.ComponentItemID.Value);

                    //actuaizar tabla de amortizacion
                    objInterfazCoreWebAmortization.CancelDocument(user.CompanyID, objTmd.ComponentItemID.Value, objTmd.Amount.Value);

                    //guardar el Saldo de la linea en el abono
            ...
            //Crear Conceptos.
            objInterfazCoreWebConcept.CancelInvoice(user.CompanyID, TransactionId.Value, TransactionMasterId.Value);
        }
    }
    ...
    public void LoadRender(TypeRender typeRedner)
    {
        ...
        switch (typeRedner)
        {
            case TypeRender.New:
                ...
                lblTitulo.Text = @"DOCUMENTO DE CANCELACION #:00000000";
                txtCustomerID = 0;
            ...
            case TypeRender.Edit:
                ...
                lblTitulo.Text = @$"DOCUMENTO DE CANCELACION #:{ObjTransactionMaster.TransactionNumber ?? string.Empty}";
                ...
                renderGridFiles = new RenderFileGridControl(user.CompanyID, ObjComponentCancelInvoice!.ComponentID, TransactionMasterId.Value);
