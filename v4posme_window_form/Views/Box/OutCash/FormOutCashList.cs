...
{
    public partial class FormOutCashList : FormTypeList, IFormTypeList
    {
        ...
        public void List()
        {
            ...
            {
                var permited = coreWebPermission.UrlPermited("app_box_outcash", "index", urlSuffix!,
            ...
            var objComponent = coreWebTools.GetComponentIdByComponentName("tb_transaction_master_outputcash");
            if (objComponent is null)
            {
                coreWebRender.GetMessageAlert(TypeError.Error, "Error", "00409 EL COMPONENTE 'tb_transaction_master_outputcash' NO EXISTE...", this);
                return;
            }
        ...
        public void Delete(object? sender, EventArgs? args)
        {
            ...
            backgroundWorker.DoWork += (ob, ev) =>
            {
                ...
                foreach (var indexRow in rowIndex)
                {
                    ...
                    var FormOutCashEdit = new FormOutCashEdit(TypeOpenForm.Init, companyID, transactionMasterID, transactionId);
                    FormOutCashEdit.ComandDelete();
                }
            };
        ...
        public void Edit(object? sender, EventArgs? args)
        {
            ...
            var transactionId = Convert.ToInt32(gridViewData.GetRowCellValue(focused, "transactionID"));
            var transactionMasterID = Convert.ToInt32(gridViewData.GetRowCellValue(focused, "transactionMasterID"));
            var frm = new FormOutCashEdit(TypeOpenForm.Init, companyID, transactionMasterID, transactionId)
            {
        ...
        public void Print(object? sender, EventArgs? args)
        {
            ...
            var transactionId = Convert.ToInt32(gridViewData.GetRowCellValue(focused, "transactionID"));
            var transactionMasterID = Convert.ToInt32(gridViewData.GetRowCellValue(focused, "transactionMasterID"));
            var FormOutCashEdit = new FormOutCashEdit(TypeOpenForm.NotInit, companyID, transactionMasterID, transactionId);
            FormOutCashEdit.ComandPrinter();
        }

        public void PreRender()
        {
            lblTitulo.Text = @"LISTA DE EGRESOS A CAJA";
            Text = lblTitulo.Text;
        ...
        public void SearchTransactionMaster(object? sender, EventArgs? args)
        {
            ...
            {
                var permited = coreWebPermission.UrlPermited("app_box_outcash", "index", urlSuffix!, VariablesGlobales.Instance.ListMenuTop, VariablesGlobales.Instance.ListMenuLeft, VariablesGlobales.Instance.ListMenuBodyReport, VariablesGlobales.Instance.ListMenuBodyTop, VariablesGlobales.Instance.ListMenuHiddenPopup);
                if (!permited)
                {
                    throw new Exception(notAccessControl);
                }

                var resultPermission = coreWebPermission.UrlPermissionCmd("app_box_outcash", "index", urlSuffix!, role, user, VariablesGlobales.Instance.ListMenuTop, VariablesGlobales.Instance.ListMenuLeft, VariablesGlobales.Instance.ListMenuBodyReport, VariablesGlobales.Instance.ListMenuBodyTop, VariablesGlobales.Instance.ListMenuHiddenPopup);
                if (resultPermission == permissionNone)
                {
                    throw new Exception(notAccessControl);
                }
            ...
            var frm = new FormOutCashEdit(TypeOpenForm.Init, objTm.CompanyID, objTm.TransactionID, objTm.TransactionMasterID)
            {
                MdiParent = CoreFormList.Principal()
            };
            frm.Show();
