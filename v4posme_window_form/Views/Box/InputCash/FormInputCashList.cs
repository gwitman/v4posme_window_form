...
{
    public partial class FormInputCashList : FormTypeList, IFormTypeList
    {
        ...
        private void FormInputCashList_Load(object sender, EventArgs e)
        {
        ...
        public void List()
        {
            ...
            {
                var permited = coreWebPermission.UrlPermited("app_box_inputcash", "index", urlSuffix!,
            ...
            var objComponent = coreWebTools.GetComponentIdByComponentName("tb_transaction_master_inputcash");
            if (objComponent is null)
            {
                coreWebRender.GetMessageAlert(TypeError.Error, "Error", "00409 EL COMPONENTE 'tb_transaction_master_inputcash' NO EXISTE...", this);
                return;
            }
        ...
        public void Delete(object? sender, EventArgs? args)
        {
            ...
            {
                coreWebRender.GetMessageAlert(TypeError.Warning, "Cancelacion Caja", "No hay datos en la tabla", this);
                return;
            }
            ...
            if (focused < 0)
            {
                coreWebRender.GetMessageAlert(TypeError.Warning, "Cancelacion Caja", "Seleccione un ingreso de caja a eliminar", this);
                return;
            }
            ...
            backgroundWorker.DoWork += (ob, ev) =>
            {
                ...
                foreach (var indexRow in rowIndex)
                {
                    ...
                    var formInputCashEdit = new FormInputCashEdit(TypeOpenForm.Init, companyID, transactionMasterID, transactionId);
                    formInputCashEdit.ComandDelete();
                }
            };
            ...
            backgroundWorker.RunWorkerCompleted += (ob, ev) =>
            {
                ...
                {
                    coreWebRender.GetMessageAlert(TypeError.Warning, "Eliminar", "Se ha cancelado la eliminación del ingreso de caja", this);
                }
                else
                {
                    coreWebRender.GetMessageAlert(TypeError.Informacion, "Eliminar", "Se ha eliminado el ingreso de caja de forma correcta", this);
                    List();
                    RefreshData();
                }
        ...
        public void Edit(object? sender, EventArgs? args)
        {
            ...
            if (focused < 0)
            {
                coreWebRender.GetMessageAlert(TypeError.Warning, "Cancelacion Factura", "Seleccione un ingreso a caja a editar", this);
                return;
            }
            ...
            var transactionId = Convert.ToInt32(gridViewData.GetRowCellValue(focused, "transactionID"));
            var transactionMasterID = Convert.ToInt32(gridViewData.GetRowCellValue(focused, "transactionMasterID"));
            var frm = new FormInputCashEdit(TypeOpenForm.Init, companyID, transactionMasterID, transactionId)
            {
        ...
        public void Print(object? sender, EventArgs? args)
        {
            ...
            var transactionId = Convert.ToInt32(gridViewData.GetRowCellValue(focused, "transactionID"));
            var transactionMasterID = Convert.ToInt32(gridViewData.GetRowCellValue(focused, "transactionMasterID"));
            var formInputCashEdit = new FormInputCashEdit(TypeOpenForm.NotInit, companyID, transactionMasterID, transactionId);
            formInputCashEdit.ComandPrinter();
        }

        public void PreRender()
        {
            lblTitulo.Text = @"LISTA DE INGRESOS A CAJA";
            Text = lblTitulo.Text;
        ...
        public void SearchTransactionMaster(object? sender, EventArgs? args)
        {
            ...
            {
                var permited = coreWebPermission.UrlPermited("app_box_inputcash", "index", urlSuffix!, VariablesGlobales.Instance.ListMenuTop, VariablesGlobales.Instance.ListMenuLeft, VariablesGlobales.Instance.ListMenuBodyReport, VariablesGlobales.Instance.ListMenuBodyTop, VariablesGlobales.Instance.ListMenuHiddenPopup);
                if (!permited)
                {
                    throw new Exception(notAccessControl);
                }

                var resultPermission = coreWebPermission.UrlPermissionCmd("app_box_inputcash", "index", urlSuffix!, role, user, VariablesGlobales.Instance.ListMenuTop, VariablesGlobales.Instance.ListMenuLeft, VariablesGlobales.Instance.ListMenuBodyReport, VariablesGlobales.Instance.ListMenuBodyTop, VariablesGlobales.Instance.ListMenuHiddenPopup);
                if (resultPermission == permissionNone)
                {
                    throw new Exception(notAccessControl);
                }
            ...
            var frm = new FormInputCashEdit(TypeOpenForm.Init, objTm.CompanyID, objTm.TransactionID, objTm.TransactionMasterID)
            {
                MdiParent = CoreFormList.Principal()
            };
            frm.Show();
