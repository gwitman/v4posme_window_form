...
{
    public partial class FormShareCapitalList : FormTypeList, IFormTypeList
    {
        ...
        private void FormShareCapitalList_Enter(object sender, EventArgs e)
        {
            //este evento es cada vez que el formulario tiene el focus
            if (!backgroundWorker.IsBusy)
            {
                FormShareCapitalList_Load(sender, e);
            }
        }

        private void FormShareCapitalList_Load(object sender, EventArgs e)
        {
        ...
        public void List()
        {
            ...
            {
                var permited = coreWebPermission.UrlPermited("app_box_sharecapital", "index", urlSuffix!,
                ...
                resultPermission = coreWebPermission.UrlPermissionCmd("app_box_sharecapital", "index", urlSuffix!,
            ...
            var objComponent = coreWebTools.GetComponentIdByComponentName("tb_transaction_master_share_capital");
            if (objComponent is null)
            {
                coreWebRender.GetMessageAlert(TypeError.Error, "Error", "00409 EL COMPONENTE 'tb_transaction_master_share_capital' NO EXISTE...", this);
                return;
            }
        ...
        public void Delete(object? sender, EventArgs? args)
        {
            ...
            {
                coreWebRender.GetMessageAlert(TypeError.Warning, "Abono Cap", "No hay datos en la tabla", this);
                return;
            }
            ...
            backgroundWorker.DoWork += (ob, ev) =>
            {
                ...
                foreach (var indexRow in rowIndex)
                {
                    ...
                    var formShareEdit = new FormShareCapitalEdit(TypeOpenForm.Init, companyID, transactionMasterID, transactionId);
        ...
        public void Edit(object? sender, EventArgs? args)
        {
            ...
            var transactionId = Convert.ToInt32(gridViewData.GetRowCellValue(focused, "transactionID"));
            var transactionMasterID = Convert.ToInt32(gridViewData.GetRowCellValue(focused, "transactionMasterID"));
            var formShareCapital = new FormShareCapitalEdit(TypeOpenForm.Init, companyID, transactionMasterID, transactionId)
            {
        ...
        {
            var formShareCapital = new FormShareCapitalEdit(TypeOpenForm.Init, 0, 0, 0)
            {
                MdiParent = CoreFormList.Principal()
            };
            formShareCapital.Show();
        ...
        {
            lblTitulo.Text = @"LISTA DE ABONOS CAP";
            Text = lblTitulo.Text;
        ...
        public void SearchTransactionMaster(object? sender, EventArgs? args)
        {
            ...
            {
                var permited = coreWebPermission.UrlPermited("app_box_sharecapital", "index", urlSuffix!, VariablesGlobales.Instance.ListMenuTop, VariablesGlobales.Instance.ListMenuLeft, VariablesGlobales.Instance.ListMenuBodyReport, VariablesGlobales.Instance.ListMenuBodyTop, VariablesGlobales.Instance.ListMenuHiddenPopup);
                if (!permited)
                {
                    throw new Exception(notAccessControl);
                }

                var resultPermission = coreWebPermission.UrlPermissionCmd("app_box_sharecapital", "index", urlSuffix!, role, user, VariablesGlobales.Instance.ListMenuTop, VariablesGlobales.Instance.ListMenuLeft, VariablesGlobales.Instance.ListMenuBodyReport, VariablesGlobales.Instance.ListMenuBodyTop, VariablesGlobales.Instance.ListMenuHiddenPopup);
                if (resultPermission == permissionNone)
                {
                    throw new Exception(notAccessControl);
                }
            ...
            var frm = new FormShareCapitalEdit(TypeOpenForm.Init, objTm.CompanyID, objTm.TransactionID, objTm.TransactionMasterID)
            {
                MdiParent = CoreFormList.Principal()
            };
            frm.Show();
