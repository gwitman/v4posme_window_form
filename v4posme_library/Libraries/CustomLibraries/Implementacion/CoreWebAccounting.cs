...
{
    private readonly ICompanyParameterModel _companyParameterModel = VariablesGlobales.Instance.UnityContainer.Resolve<ICompanyParameterModel>();
    private readonly IComponentCycleModel _componentCycleModel = VariablesGlobales.Instance.UnityContainer.Resolve<IComponentCycleModel>();
    private readonly IParameterModel _parameterModel = VariablesGlobales.Instance.UnityContainer.Resolve<IParameterModel>();
    private readonly IComponentPeriodModel _componentPeriodModel = VariablesGlobales.Instance.UnityContainer.Resolve<IComponentPeriodModel>();
    private readonly IAccountModel _accountModel = VariablesGlobales.Instance.UnityContainer.Resolve<IAccountModel>();
    private readonly IAccountingBalanceModel _accountingBalanceModel = VariablesGlobales.Instance.UnityContainer.Resolve<IAccountingBalanceModel>();
    ...
    public bool CycleIsCloseById(int companyId, int cycleId)
    {
        ...
        var objCompanyParameter = _companyParameterModel.GetRowByParameterIdCompanyId(companyId, objParameter.ParameterId);
        ...
        int componentPeriodId, int componentCycleId, decimal balance, decimal debit, decimal credit)
    {
        ...
        {
            MayorizateAccount(companyId, branchId, loginId, objAccount.ParentAccountId.Value, componentPeriodId, componentCycleId, balance, debit, credit);
        }
        _accountingBalanceModel.UpdateBalance(companyId, componentPeriodId, componentCycleId, accountId, balance, debit, credit);
    }
    public int MayorizateCycle(int companyId, int branchId, int loginId, int componentPeriodId, int componentCycleId)
    {
        ...
        var workflowStageCycleClose = _coreWebParameter.GetParameter("ACCOUNTING_CYCLE_WORKFLOWSTAGECLOSED", companyId).Value;
        ...
        _accountingBalanceModel.SetJournalSummary(companyId, branchId, loginId, componentCycleId, Convert.ToInt32(journalTypeClosed));
        //Ingresar las cuentas en la tabla balance
        _accountingBalanceModel.SetAccountBalance(companyId, branchId, loginId, componentCycleId, componentPeriodId, componentAccountId);
        //Mayorizar Cuentas
        ...
        {
            var objAccountBalance = _accountingBalanceModel.GetInfoAccount(companyId, branchId, loginId, minAccountId.Value);
