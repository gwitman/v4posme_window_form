...
class ComponentAuditDetailModel : IComponentAuditDetailModel
{
    ...
    {
        using var context = new DataContext();
        var result = from cu in context.TbComponentAudits
            join cud in context.TbComponentAuditDetails
                on new { cu.CompanyId, cu.ComponentAuditId } equals new { cud.CompanyId, cud.ComponentAuditId }
            join u in context.TbUsers on cu.ModifiedBy equals u.UserId
            join se in context.TbSubelements on cud.FieldId equals se.SubElementId
            join ws in context.TbWorkflowStages
                on Convert.ToInt32(cud.OldValue) equals ws.WorkflowStageId into wsJoin
            from ws in wsJoin.DefaultIfEmpty()
            join ws2 in context.TbWorkflowStages
                on Convert.ToInt32(cud.NewValue) equals ws2.WorkflowStageId into ws2Join
            from ws2 in ws2Join.DefaultIfEmpty()
            join ci in context.TbCatalogItems on Convert.ToInt32(cud.OldValue) equals ci.CatalogItemId into ciJoin
            from ci in ciJoin.DefaultIfEmpty()
            join ci2 in context.TbCatalogItems on Convert.ToInt32(cud.NewValue) equals ci2.CatalogItemId into ci2Join
